...
// CaddyManager handles Caddy configuration with enhanced service support
...
// ServiceConfig represents a simple service configuration (for backward compatibility)
type ServiceConfig struct {
	Hostname     string `json:"hostname"`
	Backend      string `json:"backend"` // This is still the server's internal API
...
// NewCaddyManager creates a new enhanced Caddy manager
func NewCaddyManager(adminAPI string) *CaddyManager {
	return &CaddyManager{
		adminAPI:         adminAPI,
...
func (cm *CaddyManager) AddService(hostname, backend, protocol string) error {
...
func (cm *CaddyManager) AddServiceWithWebSocket(hostname, backend, protocol string, webSocket bool) error {
...
func (cm *CaddyManager) AddFullServiceConfig(hostname, backend, protocol string, webSocket, httpRedirect bool, listenOn string) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
...
func (cm *CaddyManager) AddEnhancedService(serviceConfig *agent.ServiceConfig) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
...
func (cm *CaddyManager) RemoveService(hostname string) error {
	cm.mu.Lock()
	defer cm.mu.Unlock()
...
func (cm *CaddyManager) hasWebSocketEnabledServicesLocked() bool {
...
// determineListenPorts determines which ports to listen on based on service configurations
func (cm *CaddyManager) determineListenPorts() []string {
	needsHTTP := false
	needsHTTPS := false

	// Check what protocols are needed by examining all services
	for _, service := range cm.config {
		switch service.ListenOn {
		case "http":
			needsHTTP = true
		case "https":
			needsHTTPS = true
	...
	// If no services are configured, default to both
	if len(cm.config) == 0 {
		needsHTTP = true
		needsHTTPS = true
	}
	...
	// Ensure we always listen on at least HTTPS
	if len(ports) == 0 {
		ports = []string{":443"}
	}

	logger.Debug("🔧 Caddy listen configuration: %v (HTTP: %t, HTTPS: %t)", ports, needsHTTP, needsHTTPS)
	return ports
}
...
// buildEnhancedRoutes builds enhanced Caddy routes with support for advanced agent features (DEPRECATED)
// This function is kept for backward compatibility but not used in the new separate server approach
func (cm *CaddyManager) buildEnhancedRoutes() []map[string]interface{} {
	// This function is no longer used - we now use buildHTTPSRoutes and buildHTTPRoutes
	// for separate HTTP and HTTPS servers to properly handle redirects
	return []map[string]interface{}{}
}

// buildEnhancedServiceRoutes builds routes for enhanced service configurations
func (cm *CaddyManager) buildEnhancedServiceRoutes(service *ServiceConfig, enhancedService *agent.ServiceConfig) []map[string]interface{} {
...
func (cm *CaddyManager) buildMatchConditions(hostname string, match *agent.MatchConfig) []map[string]interface{} {
...
func (cm *CaddyManager) hasHTTPSServices() bool {
	for _, service := range cm.config {
...
func (cm *CaddyManager) hasHTTPServices() bool {
	for _, service := range cm.config {
...
func (cm *CaddyManager) buildHTTPSRoutes() []map[string]interface{} {
	var routes []map[string]interface{}
...
func (cm *CaddyManager) buildHTTPRoutes() []map[string]interface{} {
	var routes []map[string]interface{}
...
func (cm *CaddyManager) buildEnhancedHandlers(service *ServiceConfig, enhancedService *agent.ServiceConfig, routeConfig *agent.RouteConfig) []map[string]interface{} {
...
func (cm *CaddyManager) buildHeadersHandler(middleware *agent.MiddlewareConfig) map[string]interface{} {
...
func (cm *CaddyManager) buildRateLimitHandler(middleware *agent.MiddlewareConfig) map[string]interface{} {
...
func (cm *CaddyManager) buildReverseProxyHandler(service *ServiceConfig, enhancedService *agent.ServiceConfig) map[string]interface{} {
...
func (cm *CaddyManager) buildRequestHeadersHandler(service *ServiceConfig, enhancedService *agent.ServiceConfig) map[string]interface{} {
...
func (cm *CaddyManager) buildSimpleServiceRoute(service *ServiceConfig) map[string]interface{} {
...
func (cm *CaddyManager) serviceSupportsHTTPS(service *ServiceConfig) bool {
...
func (cm *CaddyManager) serviceSupportsHTTP(service *ServiceConfig) bool {
...
func (cm *CaddyManager) buildEnhancedServiceHTTPRoutes(service *ServiceConfig, enhancedService *agent.ServiceConfig) []map[string]interface{} {
	var routes []map[string]interface{}
...
func (cm *CaddyManager) buildSimpleServiceHTTPRoute(service *ServiceConfig) map[string]interface{} {
	requestHeaders := map[string][]string{
...
func (cm *CaddyManager) buildHTTPMatchConditions(hostname string, match *agent.MatchConfig) []map[string]interface{} {
	conditions := []map[string]interface{}{
		{
			"host": []string{hostname},
			// No scheme matcher - this route is only in HTTP server
