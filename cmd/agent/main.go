...
func main() {
	...
	log.Info("🤖 0Trust Agent Starting...")
	...
	log.Debug("🔍 Loaded from config file - ID=%s, Server=%s", config.Agent.ID, config.Server.Address)

	// Debug: Show command line flag values
	log.Debug("🔍 Command line flags - id=%s, server=%s, config=%s", *id, *serverAddr, *configFile)

	// Override config with command line arguments if provided
	if *id != "" {
		log.Debug("🔄 Overriding agent ID from flag: %s -> %s", config.Agent.ID, *id)
		config.Agent.ID = *id
	}
	if *serverAddr != "" {
		log.Debug("🔄 Overriding server address from flag: %s -> %s", config.Server.Address, *serverAddr)
		config.Server.Address = *serverAddr
	}
	if *certFile != "" {
		log.Debug("🔄 Overriding cert file from flag: %s -> %s", config.Server.Cert, *certFile)
		config.Server.Cert = *certFile
	}
	if *keyFile != "" {
		log.Debug("🔄 Overriding key file from flag: %s -> %s", config.Server.Key, *keyFile)
		config.Server.Key = *keyFile
	}
	if *caFile != "" {
		log.Debug("🔄 Overriding CA file from flag: %s -> %s", config.Server.CACert, *caFile)
		config.Server.CACert = *caFile
	}
	if *logLevel != "" {
		log.Debug("🔄 Overriding log level from flag: %s -> %s", config.LogLevel, *logLevel)
	...
	if level == "" {
		...
		if level != "" {
			log.Debug("🔄 Using LOG_LEVEL environment variable: %s", level)
		}
	}
	if level == "" {
		level = "INFO"
	...
	log.Debug("🎯 Final configuration - ID=%s, Server=%s", config.Agent.ID, config.Server.Address)

	log.Info("🔧 Configuration: ID=%s, Server=%s, LogLevel=%s", config.Agent.ID, config.Server.Address, level)

	// Validate required configuration
	if config.Agent.ID == "" {
		log.Error("❌ Agent ID is required (set in config file or use -id flag)")
		os.Exit(1)
	}
	if config.Server.Address == "" {
		log.Error("❌ Server address is required (set in config file or use -server flag)")
		os.Exit(1)
	}

	log.Debug("🔐 Certificate files: cert=%s, key=%s, ca=%s", config.Server.Cert, config.Server.Key, config.Server.CACert)
	...
	// Start agent
	log.Info("🚀 Starting agent %s", config.Agent.ID)
	...
	log.Info("✅ Agent is running and ready to handle requests")
	select {}
}
