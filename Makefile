...
# CI gates on EVERY severity (low+) — the codebase is fully gosec-clean.
# Known-intentional findings are suppressed inline with
# `#nosec <rule> -- <justification>` at the offending line —
...
# CI gate: fails on ANY finding not suppressed inline with #nosec.
sec:
